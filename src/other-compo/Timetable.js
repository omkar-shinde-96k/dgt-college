
function TimeTable() {
    return (
        <>



<div class="container">
                <h2>Termwise Academic Schedule</h2>

                <table class="table">
                    <thead>
                        <tr> 
                        </tr>
                    </thead>
                    <tbody>
                        <tr> 
                            <td>As per Univercity of Mumbai rules regulations</td> 
                        </tr>
                        <tr> 
                            <td> </td> 
                        </tr>
                    </tbody>
                </table>
            </div> 

            <div class="container">
                <h2>Daywise Academic Schedule</h2>

                <table class="table">
                    <thead>
                        <tr>
                            <th>No.</th>
                            <th>Timing</th>
                            <th>Academic Work </th>
                            <th>faculty</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>1</td>
                            <td>8.20 am to 1.50 pm </td>
                            <td>Theory</td>
                            <td>F.Y./S.Y./T.Y.B.A.</td>
                        </tr>
                        <tr>
                            <td>2</td>
                            <td>8.20 am to 3.00 Noon </td>
                            <td>Theory & Practical</td>
                            <td>F.Y./S.Y./T.Y.Bsc.</td>
                        </tr>
                        <tr>
                            <td>3</td>
                            <td>8.20 am to 3.00 Noon  </td>
                            <td>Theory </td>
                            <td>F.Y./S.Y./T.Y.B.com.</td>
                        </tr>
                        <tr>
                            <td>4</td>
                            <td>8.20 am to 3.00 Noon  </td>
                            <td>Theory </td>
                            <td>F.Y./S.Y./T.Y.BMS</td>
                        </tr>

                        <tr>
                            <td>5</td>
                            <td>8.20 am to 3.00 Noon  </td>
                            <td>Theory & Practical </td>
                            <td>F.Y./S.Y./T.Y.B.Sc.I.T</td>
                        </tr>
                        <tr>
                            <td>6</td>
                            <td>8.20 am to 2.00 pm</td>
                            <td>Theory & Practical </td>
                            <td>11th/12th Science</td>
                        </tr>
                        <tr>
                            <td>7</td>
                            <td>8.20 am to 12.00 pm</td>
                            <td>Theory </td>
                            <td>11th/12th Commerce</td>
                        </tr>
                        <tr>
                            <td>8</td>
                            <td> 1.00 pm to 3.00pm </td>
                            <td>Theory </td>
                            <td>M.A.1/M.A.2 Hindi/Marathi/History</td>
                        </tr>
                        <tr>
                            <td>9</td>
                            <td>8.20 pm to 2.40 pm </td>
                            <td>Theory & Practical</td>
                            <td>M.Sc 1/2</td>
                        </tr>
                        <tr>
                            <td>10</td>
                            <td> 8.20 pm to 2.40 pm</td>
                            <td>Theory & Practical</td>
                            <td>M.Sc.I.T 1/2</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <br />
            <hr />
            <br />
            <div class="container">
                <h2>Office Work schedule</h2>
                <table class="table">
                    <tbody>
                        <tr>
                            <td>9.00 am to 5.00 pm </td>
                            <td>1.00 pm to 1.30 pm Lunch Break</td>
                        </tr>
                    </tbody>
                </table>
    Student Will not entertained for office work during their schedule for theory or practical
  </div>
            <br />
            <hr />
            <br />
            <div class="container">
                <h2>Library Book Issuing Schedule</h2> 
                <table class="table">
                    <tbody>
                        <tr>
                            <td>9.00 am to 5.00 pm </td>
                            <td>Scheduled timing in library , Reading rooms available for student</td>
                        </tr>
                        <tr>
                            <td>10.00 am to 4.00 pm </td>
                            <td>Book issuing timing for All Students.</td>
                        </tr>
                        <tr>
                            <td>9.00 am to 4.00 pm </td>
                            <td>Book issuing timing for teaching - Non teaching Staff</td>
                        </tr>
                        <tr>
                            <td>9.00 am to 4.00 pm </td>
                            <td>Lunch Time</td>
                        </tr>
                    </tbody>
                </table>
            </div>

            <br />
            <hr />
            <br />

        </>
    );
}

export default TimeTable;